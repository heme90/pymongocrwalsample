...
conn = pymongo.MongoClient("127.0.0.1",27777)
chart = conn['api']['Dartchart']
chart.create_index([("rcp_no",pymongo.ASCENDING)],unique=True)
...
def dartparser_b(i):
    ...
    conurl = "https://dart.fss.or.kr/dsaf001/main.do?rcpNo=" + str(i[0])
    driver_a.get(conurl)
    dic = {}
    dic["rcp_no"]=i[0]
    dic["crp_nm"]=i[1]
    ...
    try:
        ...
        h = []
        rows = d.select("body > table > tbody > tr")
        
        for row in rows:
            da = {}
            c = row.select("td")
            rep = c[0].text.replace(" ","").replace(",","").replace(".","")
            if(rep == "" or rep[0:3] == "(단위" or rep == "<br>" or rep == "구분"):
                continue
            da["구분"] = c[0].text.replace("\n","")
            da["3rd"] = c[1].text.replace("\n","")
            da["2nd"] = c[2].text.replace("\n","")
            da["1st"] = c[3].text.replace("\n","")
            h.append(da)
            
                
        dic["list"] = h
            
        chart.insert_one(dic)
...
if __name__ == '__main__':
    ...
    con = mongoconnection()
    res = []
    for i in con.find({}, {"_id" : 0 , "list.rcp_no": 1,"list.crp_nm" : 1,"list.rcp_dt":1,"list.rpt_nm":1 }).limit(100):
        res.append((i["list"]["rcp_no"] , i["list"]["crp_nm"] , i["list"]["rcp_dt"] , i["list"]["rpt_nm"]))
    ...
    print("end")
    driver_a.close()
    os.system("killchromedriver.bat")
